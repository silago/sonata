<div id="info">
    <ul class="breadcrumb">
		...
		<li class="active">Изменение вида доставки - "{$sname}"</li>
    </ul>
</div>
{if $error}
...
{/if}
<form method="post" action="/admin/orders/editShipment.php?id={$id}">
  <fieldset>
			...
			<div class="controls">
				{foreach from=$payments item=item key=key}
					<label class="checkbox">
						<input type="checkbox" name="payment[{$key}]" value="{$item.title}" {foreach from=$payment item=item1 key=key1}{if $item.title == $item1}checked{/if}{/foreach}> {$item.name}
					</label>
				{/foreach}				
			</div>
	</div>
