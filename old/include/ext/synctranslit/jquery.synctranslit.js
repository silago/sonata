...
;(function($){
    $.fn.syncTranslit = function(options) {
        ...
        return this.each(function() {            
			...
			var urilength = $destination.attr('value').length;			
			$('#urihelp').html(($max - urilength) + ' символов осталось ' + '('+$max+' максимум)');			
            ...
            $this.keyup(function(){
				...
				var	result1 = result.substring(0, $max);							
				
				var resultArr1 = result1.split('-');									
				
				for (var i=0; i<resultArr.length; i++){
					if(resultArr[i] == resultArr1[i])
						var check = i;
				}				
				
				resultArr1.splice(check+1,1);												
				var resultEnd = resultArr1.join(''+o.urlSeparator+'');				
				$destination.val(resultEnd);				
				$('#urihelp').html(($max - resultEnd.length) + ' символов осталось ' + '('+$max+' максимум)');					
            })
        });
    };
