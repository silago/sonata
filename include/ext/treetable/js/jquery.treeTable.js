...
(function($) {
    ...
    return this.each(function() {
      $(this).addClass("treeTable").find("tbody tr").each(function() {
        ...
        if (!$(this).hasClass('initialized')) {
		  ...
		  if (isRootNode && isNaN(defaultBackgroundColor)) {
			defaultBackgroundColor = ($($(this).children("td")[options.treeColumn]).css('background-color'));												
		  }
  ...
  function getBackGroundColor(node) {
    var backgroundColor = (node[0].style.backgroundColor);    
	clr = backgroundColor.match('[0-9]{3,}');				
	var clrdiff = clr[0] - 10;
	var newcolor = 'rgb(245, 245, 245)';
	return newcolor;  
  }
  ...
  function initialize(node) {
    if(!node.hasClass("initialized")) {
      ...
      if(node.hasClass("parent")) {
		...
		var backgroundColor = getBackGroundColor(cell);
		
		
        childNodes.each(function() {
          $(this).children("td")[options.treeColumn].style.paddingLeft = padding + "px";
		  
		  $(this).children("td")[options.treeColumn].style.backgroundColor = backgroundColor;
		  $(this)[options.treeColumn].style.backgroundColor = backgroundColor;
