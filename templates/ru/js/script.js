...
$(document).ready(function(){
    ...
    $('.addToCart').click(function(){
		if (fl){
			...
			$.ajax({
				...
				success:
					$.proxy(function(json){
						$('.success%20.warning%20.attention%20.information%20.error').remove();
						if (json['redirect']){
						...
						if (json['success']){
							$('#notification').html('<div class="success" style="display: none;">'+ json['success']+'<span class="close"><img src="/images/close.png" alt="" class="close" /></span></div>');
							$('.success').fadeIn('slow');
	...
	function animateProduct(image, cart){
        ...
        fl = false;
                var tis = $(this);
                $.ajax({
        url:'index.php?route=checkout/cart/add', type:'post', data:'product_id=' + tis.attr("data-id"), dataType:'json', content:this, success:$.proxy(function(json){
        ...
        , this)}
                ...
                vertical:false, visible:3, scroll:1}
                        ...
                        fl2 = false;
                                $.ajax({
                        url:'index.php?route=checkout/cart/add', type:'post', data:'product_id=' + product_id, dataType:'json', success:function(json){
                        $('.success, .warning, .attention, .information, .error').remove();
                                if (json['redirect']){
                        location = json['redirect'];
                        }
                        ...
                        var cart = $('#cart').offset();
                                ...
                                params = {
                        top:cart.top + 'px', left:cart.left + 'px', opacity:0.2, width:$('#img_' + product_id).width(), height:$('#img_' + product_id).height()}
                        ;
                                $('#temp').animate(params, 'slow', false, function(){
                        $('#temp').remove();
        fl2 = true;
