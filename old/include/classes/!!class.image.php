...
class image {
	...
	private function checkWritible($file, $folder) {
		if (!file_exists($file)) {
  			$this->error = "Can't fined file ".$file.", may be uploading error";
  			return false;
  		}
	...
	public function resizeEx($src, $imageBig, $imageThumb=array()) {// $imageBig add $imageThumb is array of (DstDir, MaxWidth, MaxHeight, [grauscale])
		
		if (!$this->checkWritible($src, $imageBig[0])) return false;
		
		if (!empty($imageThumb[0])) {
			if (!$this->checkWritible($src, $imageThumb[0])) return false;	
		}
		...
		$dstDir = $imageBig[0];		
		$maxWidth = $imageBig[1];
		$maxHeight = $imageBig[2];
		$width = $size[0];
		$height = $size[1];
		$fName = $this->genFileName();
		$fName .= $this->getImageExt($mime);
		...
		/*if (!empty($imageThumb[0])) {
			if ($imageThumb[1] >= $width && $imageThumb[2] >= $height) {
				$this->error = 'Error: image is to small for upload';
				return false;
			}
		}*/
		...
		// crate a thumbnail
		//$this->createThumbnail($src, fName, $imageThumb);
		
		// if src image smaller then it needed just save w/o resize
		/*if ($maxWidth >= $width && $maxHeight >= $height)	{
			$data	= file_get_contents($src);
			file_put_contents($dstDir . $fName, $data);
		}
		else {*/		
			...
			ImageCopyResampled($dstImg, $srcImg, 0, 0, 0, 0, $tnWidth, $tnHeight, $width, $height);
			...
			/* grayscale image */
			if (isset($imageBig[3]) && $imageBig[3] == true) {
		...
		if (!empty($imageThumb[0])) {
			...
			if (isset($imageThumb[3]) && $imageThumb[3] == false)
				$this->createThumbnail($src, $fName, $imageThumb, false);
			else
				$this->createThumbnail($src, $fName, $imageThumb);
		}
