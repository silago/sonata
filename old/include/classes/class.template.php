...
class template {
		...
		static function genTree1($tableName, $idCol = "group_id", $ownCol = "parent_group_id", $titCol = "name", $start = 0, $halt = -1, $orderBy = "") {
			...
			//echo "SELECT `".$idCol."`, `".$titCol."` FROM ".$API['config']['mysql']['prefix'].$tableName." WHERE ".$ownCol." = '".$start."'".(!empty($orderBy) ? " ORDER BY `".$orderBy."`" : "");
			while ($result = mysql_fetch_array($sqlId)) {
				$id    = $result[0];
				$title = $result[1];
				$parent = $result[2];
		...
		static function genTreeEval($tableName, $idCol = "id", $ownCol, $titCol, $start, $halt, $level, $orderBy) {
			global $API, $return, $lang;
                        ...
                        $sql_query = "SELECT `".$idCol."`, `".$titCol."`, `".$ownCol."`, `hidden` FROM ".$API['config']['mysql']['prefix'].$tableName." WHERE ".$ownCol." = '".$start."'".(!empty($orderBy) ? " ORDER BY `".$orderBy."`" : "");
